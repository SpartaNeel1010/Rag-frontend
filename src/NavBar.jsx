...
const Navbar = ({ activeIndex, setActiveIndex,fetchChats }) => {
    ...
    const handleCreateIndex = async (e) => {
        ...
        try {
          ...
          const response = await fetch('https://chatbot-ohana-backend-447682071256.us-central1.run.app//createindex', {
            method: 'POST',
            headers: {
    ...
    useEffect(() => {
        const fetchIndexes = async () => {
            try {
                setIsLoading(true);
                const response = await fetch("https://chatbot-ohana-backend-447682071256.us-central1.run.app//getindexes")
                const data = await response.json();
